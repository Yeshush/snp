...
long int withdraw(int branchNr, int accountNr, long int value) {
    int rv, tmp;
    rv = 0;
    pthread_mutex_lock(&(Bank[branchNr].branchLock)); // Lock the branch
    pthread_mutex_lock(&(Bank[branchNr].accounts[accountNr].acntLock)); // Lock the account
    tmp = Bank[branchNr].accounts[accountNr].balance - value;
    if (tmp >= 0) {
        Bank[branchNr].accounts[accountNr].balance = tmp;
        rv = value;
    }
    pthread_mutex_unlock(&(Bank[branchNr].accounts[accountNr].acntLock)); // Unlock the account
    pthread_mutex_unlock(&(Bank[branchNr].branchLock)); // Unlock the branch
    return rv;   
}
...
void transfer(int fromB, int toB, int accountNr, long int value) {
    pthread_mutex_lock(&(Bank[fromB].branchLock)); // Lock the source branch
    pthread_mutex_lock(&(Bank[toB].branchLock)); // Lock the destination branch
    //deadlock prevention
    if (fromB < toB) {
    ...
    pthread_mutex_unlock(&(Bank[fromB].branchLock)); // Unlock the source branch
    pthread_mutex_unlock(&(Bank[toB].branchLock)); // Unlock the destination branch
}
